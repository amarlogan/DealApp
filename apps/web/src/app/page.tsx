...
export default function Home() {
  ...
  // Load deals — from /api/deals (Supabase) with fallback to deals.json
  useEffect(() => {
    fetch("/api/deals?limit=48")
      .then(r => r.json())
      .then(data => {
      ...
      .catch(() =>
        fetch("/data/deals.json").then(r => r.json()).then(setDeals).catch(console.error)
      );
  }, []);
    ...
    <div className="animate-in w-full space-y-6 pb-10">
        ...
        <div className="relative z-10 p-8 lg:p-14 flex flex-col lg:flex-row items-center justify-between gap-8">
          <div className="text-white max-w-xl">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <a href="/category/electronics" className="bg-white/15 hover:bg-white/25 text-white px-6 py-3 rounded-full font-semibold border border-white/30 transition-all">
                ⚡ Electronics
              </a>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {CATEGORY_TILES.map(tile => (
          <a key={tile.id} href={`/category/${tile.id}`}
